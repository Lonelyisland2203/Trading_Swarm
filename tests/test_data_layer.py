...
class TestRegimeClassifier:
    ...
    def test_regime_classification(self, sample_close_series):
        ...
        classifier = RegimeClassifier(
            risk_off_threshold=75.0, risk_on_threshold=25.0, lookback_period=30
        )
    ...
    def test_pct_change_no_future_warning(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            pct_warnings = [
                x
                for x in w
                if issubclass(x.category, FutureWarning) and "pct_change" in str(x.message)
            ]
...
class TestMarketContextIntegration:
    ...
    async def test_get_market_context_with_derivatives(self, mock_cache, mock_exchange_class):
    ...
    async def test_get_market_context_spot_only_fallback(self, mock_cache, mock_exchange_class):
        ...
        result = await service.get_market_context(symbol="DOGE/USDT", timeframe="1h", limit=100)
